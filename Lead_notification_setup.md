...
## This project (Tax Expert Witness)

| Setting | Value |
|---------|--------|
| **`BRAND_NAME`** | `Tax Expert Witness` |
| **`domain`** (when `NEXT_PUBLIC_SITE_URL` is set) | `taxexpertwitness.co.uk` |
| Contact webhook | **`/api/submit-lead`** (primary; hard-fail if configured and fails) |
| Google Sheets | Soft-fail append on **`/api/submit-lead`** — one tab (`GOOGLE_SHEET_TAB_NAME`) + **Form Type** column |
